...
class TestDatevExport(TestBase):
    ...
    def _check_filecontent(self, export, refund=False):
        ...
        errors = export._validate_invoices(export.invoice_ids)
        self.assertFalse(errors, "Validation errors: %s" % errors)
    ...
    def test_missing_attachment_saves_error_messages(self):
        ...
        # Verify that NO ZIP attachment was created
        self.assertFalse(
            datev_export.attachment_id,
            "Failed export must NOT have ZIP attachment",
        )
    ...
    def test_bu_code_export_disabled(self):
        ...
        tax_with_bu_code = self.env["account.tax"].create({
            ...
            "amount_type": "percent",
            "type_tax_use": "purchase",
            "l10n_de_datev_code": "0009",  # BU-Code set
        })
    ...
    def test_bu_code_export_enabled(self):
        ...
        datev_export = self.DatevExportObj.create({
            ...
            "check_xsd": False,  # Disable XSD validation (currency issues)
        })
        datev_export.create_zip()
        ...
        with zipfile.ZipFile(fp, "r") as z:
            ...
            # Verify bu_code attribute contains the BU-Code
            bu_code_value = accounting_info.get("bu_code")
            self.assertEqual(
                bu_code_value,
                "0009",  # Expected: BU-Code as stored (with leading zeros)
                f"bu_code should be '0009' when export_bu_code=True, got: {bu_code_value}"
            )

    def test_50_export_with_validation_errors_sets_state_failed(self):
        """Test that export with validation errors sets state to 'failed' and removes ZIP"""
        _logger.info("=" * 80)
        _logger.info("TEST: Export with validation errors should set state='failed'")
        _logger.info("=" * 80)
        ...
        tax_7 = self.env["account.tax"].create({
            "name": "Test Tax 7%",
            "amount": 7.0,
            "type_tax_use": "sale",
        })

        # Create invoice with MULTIPLE taxes on one line - this violates DATEV rules
        invoice = self.InvoiceObj.create({
            "move_type": "out_invoice",
            ...
            "invoice_line_ids": [(0, 0, {
                ...
                "price_unit": 1000.0,
                "account_id": self.account_income.id,
                "tax_ids": [(6, 0, [tax_19.id, tax_7.id])],  # Multiple taxes = ERROR
            })]
        })
        invoice.action_post()
        ...
        # Run create_zip - should fail due to multiple taxes
        datev_export.create_zip()

        # Verify results
        _logger.info("Export state: %s", datev_export.state)
        _logger.info("Problematic invoices count: %s", datev_export.problematic_invoices_count)
        _logger.info("Exception info: %s", datev_export.exception_info)
        _logger.info("Has attachment: %s", bool(datev_export.attachment_id))

        # Assertions: When exceptions occur during get_zip(),
        # the state is set to 'failed' and exception_info is populated,
        # but problematic_invoices_count may remain 0 (since validation didn't reach that stage)
        self.assertEqual(
            datev_export.state,
            "failed",
            "Export with validation errors must have state='failed'"
        )
        ...
        self.assertTrue(
            datev_export.exception_info,
            "Failed export should have exception_info explaining the failure"
        )

    def test_51_export_without_validation_errors_sets_state_done(self):
        """Test that export without validation errors sets state to 'done' with ZIP"""
        _logger.info("=" * 80)
        _logger.info("TEST: Export without validation errors should set state='done'")
        _logger.info("=" * 80)

        # Create analytic account (Kostenstelle) to avoid validation warnings
        analytic_plan = self.env['account.analytic.plan'].create({
            'name': 'Test Plan for Test 51',
        })
        analytic_account = self.env['account.analytic.account'].create({
        ...
        # Create a valid invoice with analytic account
        invoice = self.InvoiceObj.create({
            "move_type": "out_invoice",
            "partner_id": self.customer_de.id,
            "invoice_date": self.today,
        ...
        # Attach PDF (required for DATEV export) - use datas from existing attachment
        self.AttachmentObj.create({
            "name": f"{invoice.name}.pdf",
            "datas": self.vendor_bill_attachment_DE.datas,
            "res_model": "account.move",
            "res_id": invoice.id,
            "mimetype": "application/pdf",
        })

        # Create export (with check_xsd=False to avoid XSD validation issues)
        datev_export = self.DatevExportObj.create({
            "date_start": self.today,
            "date_stop": self.today,
            "check_xsd": False,  # Disable XSD validation to focus on basic ZIP creation
            "include_out_invoice": True,
        })
        ...
        # Assertions
        self.assertEqual(datev_export.state, "done",
                         "Export state should be 'done' when no validation errors exist")
        self.assertEqual(datev_export.problematic_invoices_count, 0,
                         "Should have 0 problematic invoices")
        self.assertTrue(datev_export.attachment_id,
                        "Export should have ZIP attachment when successful")
        self.assertFalse(datev_export.exception_info,
                         "Export should NOT have exception_info when successful")

    def test_52_failed_export_has_no_zip_attachment(self):
        """Test that failed exports do not create/keep ZIP attachments"""
        _logger.info("=" * 80)
        _logger.info("TEST: Failed export must not have ZIP attachment")
        _logger.info("=" * 80)

        # Create tax records for multiple tax error scenario
        tax_19 = self.env["account.tax"].create({
            "name": "Test Tax 19%",
            "amount": 19.0,
            "type_tax_use": "sale",
        ...
        # Create invoice with MULTIPLE taxes - violates DATEV rules
        invoice = self.InvoiceObj.create({
            "move_type": "out_invoice",
            "partner_id": self.customer_de.id,
            "invoice_date": self.today,
        ...
        # Run create_zip - should fail due to validation errors
        datev_export.create_zip()

        # Refresh record to get latest state
        datev_export = self.DatevExportObj.browse(datev_export.id)

        # Log current state for debugging
        _logger.info("Export state: %s", datev_export.state)
        _logger.info("Problematic invoices: %s", datev_export.problematic_invoices_count)
        _logger.info("Has attachment: %s", bool(datev_export.attachment_id))
        _logger.info("Exception info: %s", datev_export.exception_info)

        # Main assertions: failed exports MUST NOT have ZIP files
        self.assertEqual(
            datev_export.state,
            "failed",
            "Export with validation errors must have state='failed'"
        ...
        self.assertFalse(
            datev_export.attachment_id,
            "Failed export MUST NOT have ZIP attachment. "
            "ZIP files should only exist for successful exports."
        )

        # Note: problematic_invoices_count is only set when action_validate()
        # identifies validation issues. When an exception is thrown in get_zip(),
        # the exception_info is set but problematic_invoices_count may remain 0.
        # Therefore we only check exception_info, not problematic_invoices_count.
    ...
    # ------------------------------------------------------------------
    # _report_exception_to_detalex / action_report_exception
    # ------------------------------------------------------------------

    def test_report_exception_sets_flag_on_success(self):
        """_report_exception_to_detalex sets exception_reported=True on success."""
        datev_export = self.DatevExportObj.create({
            "date_start": self.today,
            "date_stop": self.today,
        })
        datev_export.write({
            "state": "failed",
            "exception_info": "Test error message",
        })
    ...
    def test_report_exception_does_not_crash_on_network_error(self):
        """_report_exception_to_detalex must never raise, even on network errors."""
        datev_export = self.DatevExportObj.create({
            "date_start": self.today,
            "date_stop": self.today,
        })
        datev_export.write({
            "state": "failed",
            "exception_info": "Test error",
        })

        with patch("urllib.request.urlopen", side_effect=ConnectionError("no network")):
            datev_export._report_exception_to_detalex()

        self.assertFalse(datev_export.exception_reported)

    def test_action_report_exception_raises_without_exception_info(self):
        """action_report_exception raises UserError when no exception_info."""
        datev_export = self.DatevExportObj.create({
            "date_start": self.today,
            "date_stop": self.today,
        })
        with self.assertRaises(UserError):
            datev_export.action_report_exception()

    def test_exception_reported_resets_on_draft(self):
        """exception_reported is cleared when export is set back to draft."""
        datev_export = self.DatevExportObj.create({
            "date_start": self.today,
            "date_stop": self.today,
        })
        ...
        datev_export.action_draft()
        self.assertFalse(datev_export.exception_reported)

    def test_auto_report_does_not_break_get_zip(self):
        """Auto-report failure must not break the get_zip flow."""
        bill = self.create_in_invoice(
            vendor=self.partner_DE,
            start_date=self.today,
            end_date=self.today,
    ...
    def test_validate_invoices_no_invoices(self):
        """Validation returns error when no invoices are provided."""
        generator = self.env["datev.zip.generator"]
        errors = generator._validate_invoices(self.env["account.move"])
        self.assertTrue(errors, "Should report error when no invoices provided")
        self.assertIn("Keine Rechnungen", str(errors))

    def test_validate_invoices_missing_bill_attachment(self):
        """Validation catches vendor bills without PDF attachment."""
        bill = self.create_in_invoice(
            vendor=self.partner_DE,
            start_date=self.today,
            end_date=self.today,
        ...
        generator = self.env["datev.zip.generator"]
        errors = generator._validate_invoices(bill)
        self.assertTrue(errors, "Should report error for bill without PDF")
        self.assertIn("Kein PDF-Anhang gefunden", str(errors))
        self.assertIn(bill.name, str(errors))

    def test_validate_invoices_out_invoice_without_attachment_no_error(self):
        """Out invoices without attachment pass validation (PDF can be generated)."""
        invoice = self.create_out_invoice(
            self.customer_de, self.start_date, self.end_date
        )
        ...
        generator = self.env["datev.zip.generator"]
        errors = generator._validate_invoices(invoice)
        # Out invoices can auto-generate PDF, so no error expected (if report exists)
        self.assertFalse(errors, "Out invoice without attachment should pass if report exists")

    def test_validate_invoices_valid_invoice_no_errors(self):
        """Validation returns empty list for valid invoices."""
        invoice = self.create_out_invoice(
            self.customer_de, self.start_date, self.end_date
        )
        self.create_invoice_attachment(invoice)
        ...
        generator = self.env["datev.zip.generator"]
        errors = generator._validate_invoices(invoice)
        self.assertFalse(errors, "Valid invoice should have no validation errors")

    def test_validate_invoices_collects_all_errors(self):
        """Validation collects errors from ALL invoices, not just the first."""
        bill1 = self.create_in_invoice(
            vendor=self.partner_DE,
            start_date=self.today,
            end_date=self.today,
            attachment=False,
        )
        bill2 = self.create_in_invoice(
            vendor=self.partner_DE,
            start_date=self.today,
            end_date=self.today,
            attachment=False,
        )
        for bill in (bill1, bill2):
            self.env["ir.attachment"].search(
                [("res_id", "=", bill.id), ("res_model", "=", "account.move")]
            ).unlink()

        generator = self.env["datev.zip.generator"]
        errors = generator._validate_invoices(bill1 | bill2)
        self.assertGreaterEqual(len(errors), 2,
                                "Should collect errors from all invoices, not stop at first")
        self.assertIn(bill1.name, str(errors))
        self.assertIn(bill2.name, str(errors))

    def test_generate_zip_returns_none_on_validation_failure(self):
        """generate_zip returns None when validation fails, state is 'failed'."""
        bill = self.create_in_invoice(
            vendor=self.partner_DE,
            start_date=self.today,
            end_date=self.today,
        ...
        generator = self.env["datev.zip.generator"]
        result = generator.generate_zip(datev_export.invoice_ids, False, datev_export)

        self.assertIsNone(result, "generate_zip must return None on validation failure")
        self.assertEqual(datev_export.state, "failed")
        self.assertTrue(datev_export.exception_info)
        self.assertIn("Kein PDF-Anhang gefunden", datev_export.exception_info)
