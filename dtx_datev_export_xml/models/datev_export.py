...
https://github.com/Detalex-GmbH/odoo-datev-xml-export/blob/17.0/dtx_datev_export/README_BU_CODE.md"""
    ...
    exception_reported = fields.Boolean(default=False, readonly=True, help='True when exception was reported to Detalex support.')
    ...
    def _report_exception_to_detalex(self):
        ...
        try:
            ...
            data = urllib.parse.urlencode({'arg0': payload}).encode('utf-8')
            req = urllib.request.Request('https://detalex.de/client_log', data=data, method='POST')
            req.add_header('Content-Type', 'application/x-www-form-urlencoded')
            with urllib.request.urlopen(req, timeout=10):
                pass
            ...
            _logger.warning("[EXPORT] Could not report exception to Detalex: %s", e)
            ...
            if not all(
                ...
                raise UserError(
                    _(
                        ...
                        invalid_types=", ".join(invoices_types),
