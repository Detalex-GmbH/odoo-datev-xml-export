...
The main export usage will be done by the **DATEV XML Export** Wizard, which can be executed from the menu item located at
...
Click on the button **Create DATEV XML Export File** to create the zip file for the selected filter at background.
...
On errors, an activity will be applied to you for fixing the problem. If you have fixed the problem, just restart the wizard, or click on „Recreate DATEV XML Export File“. On success you can download the generated zip file from the wizards **Download DATEV File** section.
